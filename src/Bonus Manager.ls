property pcamera, pBonusArray, pBonusTypes, pForcedActivationDistance

on new me, fCamera
  pcamera = fCamera
  pBonusTypes = []
  pBonusArray = []
  pForcedActivationDistance = VOID
  return me
end
...
on SetForcedActivationDistance me, kValue
  pForcedActivationDistance = kValue
end
...
on AddBonus me, fPosition, fTokenId, fBonusType, fActivationDistance, fRespawn, fZPos, fZOffset, fMove, fOrient, fOrientation, fToRemove, fShadowZ, fCustomRotation, fUserData
  if not voidp(pForcedActivationDistance) then
    fActivationDistance = pForcedActivationDistance
  end if
  ...
  lBonusElement = new(script("bonus"), fPosition, lRotation, fBonusType, fActivationDistance, me.pBonusTypes[fBonusType], fRespawn, fZPos, fMove, #sphere, fZOffset, lUserData, fToRemove, fShadowZ)
