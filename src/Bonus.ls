...
on Place me, fIndex, fActive
  ...
  if me.pShadowZ <> -1 then
    ...
    lModelList = gGame.GetCullingManager().GetCullerBlockModels(lPosition)
  ...
  me.pModelRef = gGame.Get3D().newModel(lName, pBonusProperties.ModelResourceRef)
  me.pModelRef.shaderList = pBonusProperties.ModelShaderRef
  me.pModelRef.transform.rotation = me.pRotation
  me.pModelRef.transform.position = me.pPosition
  if me.pZPos <> "none" then
    me.pModelRef.transform.position.z = me.pZPos
  else
    me.pZPos = me.pModelRef.transform.position.z
  end if
  me.pActive = fActive
